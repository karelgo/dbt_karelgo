...
benefits as (
    select
        ...
        duration_months,
        case 
            when lower(trim(transition_to_work)) in ('yes','y','true','1') then 1
            when lower(trim(transition_to_work)) in ('no','n','false','0') then 0
            else null
...
select
    ...
    case 
        when benefits.end_date is not null then 
            CAST((julianday(benefits.end_date) - julianday(benefits.start_date)) / 30.44 AS INTEGER)
    ...
    CASE 
        WHEN demographics.demographics_loaded_at >= benefits.benefits_loaded_at THEN CAST(demographics.demographics_loaded_at AS timestamp)
        ELSE CAST(benefits.benefits_loaded_at AS timestamp)
